...
with open('LinearRegression26147', 'rb') as file:
    secondaryModel = pickle.load(file)

for coeff in secondaryModel.coef_:
    print(coeff)
exit()
...
for i in range(1_000):
    ...
    # print('Actual total value added: {}'.format(totalValueAdded))
    
    testX = np.array([list(dataItem['features'].values())])
    predictedTotalValueAdded = secondaryModel.predict(testX)
    # print('Predicted total value added: {}'.format(predictedTotalValueAdded))

    print(','.join([str(totalValueAdded), str(predictedTotalValueAdded[0])]))
